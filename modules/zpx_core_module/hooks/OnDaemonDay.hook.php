...
while ($modules = $modsql->fetch()) {
    ...
    $mod_config = new xml_reader(fs_filehandler::ReadFileContents(ctrl_options::GetSystemOption('ractpanel_root') . 'modules/' . $modules['mo_folder_vc'] . '/module.xml'));
    $mod_config->Parse();
...
/*
 * Please DO NOT remove the below code, this helps us at the RactPanel project
 * find out non-personal infomation about how people are running RactPanel. The only infomation
 * that we are passing back here is your RactPanel version and what OS you are running it on in addition
...
ws_generic::DoPostRequest('http://api.ractpanel.ractbd.com/hello.json', "version=" . sys_versions::ShowRactPanelVersion() . "&platform=" . sys_versions::ShowOSPlatformVersion() . "&url=" . ctrl_options::GetSystemOption('ractpanel_domain') . "mail=" . $zadmin['email']);
