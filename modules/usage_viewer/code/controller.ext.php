...
class module_controller extends ctrl_module
{
    ...
    static function build_row_usage($name, $used, $quota, $human = false)
    {
        ...
        if ($quota < 0) {
            ...
            $res .= ' / ' . (($human) ? fs_director::ShowHumanFileSize($quota) : $quota) . '</td><td><img src="etc/lib/pChart2/ractpanel/zProgress.php?percent=' . (($quota == 0 or $used == $quota) ? 100 : round($used / $quota * 100, 0)) . '"/>';
        }
        return $res . '</td></tr>';
    }
    ...
    static function DisplayUsagepChart()
    {
        ...
        $line = '<table class="none" cellpadding="0" cellspacing="0">' .
                ...
                '<img src="etc/lib/pChart2/ractpanel/z3DPie.php?score=' . $free . '::' . $used .
    ...
    static private function DisplayChart($name, $used, $maximum)
    {
        ...
        if ($maximum < 0) { //-1 = unlimited
            ...
            $free = max($maximum - $used, 0);
            $res = '<img src="etc/lib/pChart2/ractpanel/z3DPie.php?score=' . $free . '::' . $used
    ...
    static function DisplaypBar($total, $quota)
    {
        ...
        if (fs_director::CheckForEmptyValue($type))
            return '<img src="etc/lib/pChart2/ractpanel/zProgress.php?percent=0"/>';
        if ($type == $typequota)
            return '<img src="etc/lib/pChart2/ractpanel/zProgress.php?percent=100"/>';
        return '<img src="etc/lib/pChart2/ractpanel/zProgress.php?percent=' . round($type / $typequota * 100, 0) . '"/>';
    }
