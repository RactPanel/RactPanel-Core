...
function BuildVhostPortForward($vhostName, $customPort, $userEmail)
{
    ...
    $line .= "<virtualhost *:" . ctrl_options::GetSystemOption('ractpanel_port') . ">" . fs_filehandler::NewLine();
...
function WriteVhostConfigFile()
{
    ...
    //Get email for server admin of RactPanel
    $getserveremail = $zdbh->query("SELECT ac_email_vc FROM x_accounts where ac_id_pk=1")->fetch();
    $serveremail = ( $getserveremail['ac_email_vc'] != "" ) ? $getserveremail['ac_email_vc'] : "postmaster@" . ctrl_options::GetSystemOption('ractpanel_domain');

    $VHostDefaultPort = ctrl_options::GetSystemOption('apache_port');
    $customPorts = array(ctrl_options::GetSystemOption('ractpanel_port'));
    ...
    $line .= "# Automatically generated by RactPanel " . sys_versions::ShowRactPanelVersion() . fs_filehandler::NewLine();
    $line .= "# Generated on: " . date(ctrl_options::GetSystemOption('ractpanel_df'), time()) . fs_filehandler::NewLine();
    $line .= "#==== YOU MUST NOT EDIT THIS FILE : IT WILL BE OVERWRITTEN ====" . fs_filehandler::NewLine();
    $line .= "# Use RactPanel Menu -> Admin -> Module Admin -> Apache config" . fs_filehandler::NewLine();
    $line .= "################################################################" . fs_filehandler::NewLine();
    $line .= fs_filehandler::NewLine();
    ...
    $line .= fs_filehandler::NewLine();
    $line .= "# Configuration for RactPanel control panel." . fs_filehandler::NewLine();
    $line .= "<VirtualHost *:" . ctrl_options::GetSystemOption('ractpanel_port') . ">" . fs_filehandler::NewLine();
    $line .= "ServerAdmin " . $serveremail . fs_filehandler::NewLine();
    $line .= 'DocumentRoot "' . ctrl_options::GetSystemOption('ractpanel_root') . '"' . fs_filehandler::NewLine();
    $line .= "ServerName " . ctrl_options::GetSystemOption('ractpanel_domain') . fs_filehandler::NewLine();
    $line .= 'ErrorLog "' . ctrl_options::GetSystemOption('log_dir') . 'ractpanel-error.log" ' . fs_filehandler::NewLine();
    $line .= 'CustomLog "' . ctrl_options::GetSystemOption('log_dir') . 'ractpanel-access.log" ' . ctrl_options::GetSystemOption('access_log_format') . fs_filehandler::NewLine();
    $line .= 'CustomLog "' . ctrl_options::GetSystemOption('log_dir') . 'ractpanel-bandwidth.log" ' . ctrl_options::GetSystemOption('bandwidth_log_format') . fs_filehandler::NewLine();
	...
	$errorpages = ctrl_options::GetSystemOption('ractpanel_root') . "/etc/static/errorpages";
    ...
    $line .= '<Directory "' . ctrl_options::GetSystemOption('ractpanel_root') . '">' . fs_filehandler::NewLine();
    ...
    $line .= fs_filehandler::NewLine();
    $line .= "################################################################" . fs_filehandler::NewLine();
    $line .= "# RactPanel generated VHOST configurations below....." . fs_filehandler::NewLine();
    $line .= "################################################################" . fs_filehandler::NewLine();
    $line .= fs_filehandler::NewLine();
    ...
    // RactPanel virtual host container configuration
    ...
    while ($rowvhost = $sql->fetch()) {
        ...
        if ($rowvhost['vh_enabled_in'] == 1 && ctrl_users::CheckUserEnabled($rowvhost['vh_acc_fk']) ||
            ...
            if ($vhostuser['diskquota'] != 0 && $diskspace > $vhostuser['diskquota']) {
                 ...
                 */
                // Bandwidth Settings
                //$line .= "Include C:/RactPanel/bin/apache/conf/mod_bw/mod_bw/mod_bw_Administration.conf" . fs_filehandler::NewLine();
