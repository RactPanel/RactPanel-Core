...
function ExecuteBackup($userid, $username, $download = 0) {
    ...
    $basedir = ctrl_options::GetSystemOption('ractpanel_root') . "etc/tmp/";
    if (!is_dir($basedir)) {
        fs_director::CreateDirectory($basedir);
    }
    $temp_dir = ctrl_options::GetSystemOption('ractpanel_root') . "etc/tmp/";
    ...
    } else {//cd /var/ractpanel/hostdata/zadmin/; zip -r backups/backup.zip public_html/
