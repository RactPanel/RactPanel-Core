...
/**
 *
 * RactPanel - A Cross-Platform Open-Source Web Hosting Control panel.
 *
 * @package ZPanel
 * @version $Id$
 ...
 *
 * This program (RactPanel) is free software: you can redistribute it and/or modify
...
class module_controller extends ctrl_module
{
    ...
    static function getLastRunTime()
    {
        ...
        if ($time != '0') {
            return date(ctrl_options::GetSystemOption('ractpanel_df'), $time);
        } else {
            return false;
        }
    }
    ...
    static function getNextRunTime()
    {
        if (ctrl_options::GetSystemOption('daemon_lastrun') > 0) {
            ...
            return date(ctrl_options::GetSystemOption('ractpanel_df'), $new_time);
        } else {
            // The default cron is set to run every 5 minutes on the 5 minute mark!
            return date(ctrl_options::GetSystemOption('ractpanel_df'), ceil(time() / 300) * 300);
        }
    }
    ...
    static function doUpdateConfig()
    {
        ...
        if ($numrows->fetchColumn() <> 0) {
            ...
            self::SetWriteApacheConfigTrue(); #ractpanel apache port changed require rewrite of vhosts
        }
        self::$ok = true;
    }
