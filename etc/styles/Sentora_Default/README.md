# RactPanel Default Theme #
/* RactPanel is a GPL fork of the Sentora Project */:

This document outlines some of the JavaScript and CSS functionality that is built into the RactPanel theme.  You can use any of this in your own modules and themes that you build, as long as all the proper files are loaded.

If you have any questions about this theme, please ask on the [RactPanel Forums](http://forums.ractpanel.ractbd.com/) or contact [Jason Davis](http://www.codedevelopr.com).

## Override Module Icons ##
You can replace the module icons that come with a module on the RactPanel dashboard screen with your own icons.

To override a module's icon, simply create a folder and image file in this location...

    /etc/styles/RactPanel_Default/images/MODULE-NAME/assets/icon.png
...
An example to over-ride the MySQL Database module's icon...
    ...
    /etc/styles/RactPanel_Default/images/mysql_databases/assets/icon.png

## Console Logging ##
You can log to the Browsers JavaScript Console for debugging using this function below.

    ractpanel.utils.log('my debug data');

As long as this Property `ractpanel.utils.settings.debug = false;` is set to `TRUE` then the all data will print to console.  
...
### Panels ###
Panels are one of my favorite new things in Bootstrap 3 and they are used a lot in RactPanel!
